...
import { Sparkles, Send, Bot, User, RefreshCw, AlertCircle, ChevronDown, ExternalLink, ArrowDown } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";

const SUGGESTED_QUESTIONS = [
  "Tell me about Xynox AI",
...
Ask me about Qayoom's work, projects, technical skills, experience, or how to get in touch.

Here are a few suggested topics:
- 🚀 **Featured Projects**: Xynox AI, InboxIQ AI, Game Galaxy Hub
- 💻 **Tech Stack**: Full Stack AI Engineering, React, Next.js, Node.js, RAG, Qdrant, LLMs
- 📬 **Contact & Links**: [GitHub](https://github.com/test-Ois), [LinkedIn](https://www.linkedin.com/in/qayoom-akhtar), [Resume](https://drive.google.com/drive/u/0/folders/1WayKbomGqVUlmhQbi9Y2GgXN-Q2V1IDL)`,
...
// Markdown component renderers (Resume-style typography - ZERO CARDS ON LIST ITEMS OR TECH NAMES!)
const markdownComponents = {
  ...
  p: ({ children }) => <p className="mb-2.5 last:mb-0 leading-relaxed text-gray-200 border-none bg-transparent p-0 m-0">{children}</p>,
  ul: ({ children }) => <ul className="list-disc pl-5 mb-3 space-y-1 text-left text-gray-200 border-none bg-transparent p-0 m-0">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal pl-5 mb-3 space-y-1 text-left text-gray-200 border-none bg-transparent p-0 m-0">{children}</ol>,
  li: ({ children }) => <li className="leading-relaxed text-gray-200 border-none bg-transparent p-0 m-0">{children}</li>,
  strong: ({ children }) => <strong className="font-semibold text-white">{children}</strong>,
  code: ({ inline, className, children, ...props }) => {
    if (inline) {
      return (
        <code className="px-1.5 py-0.5 rounded text-[11px] font-mono bg-purple-950/60 text-purple-300 border border-purple-800/40" {...props}>
          {children}
        </code>
      );
    }
  ...
  h1: ({ children }) => <h1 className="text-base font-bold text-white mb-2 mt-3.5 border-none bg-transparent p-0">{children}</h1>,
  h2: ({ children }) => <h2 className="text-sm font-bold text-white mb-2 mt-3 border-none bg-transparent p-0">{children}</h2>,
  h3: ({ children }) => <h3 className="text-xs font-bold text-white mb-1.5 mt-2.5 border-none bg-transparent p-0">{children}</h3>,
};

// Exact single fixed thinking status label (NO timers, NO phrase rotation)
const getThinkingLabel = (message) => {
  const text = (message || "").toLowerCase();

  if (text.includes("xynox")) {
    return "Looking up Xynox AI...";
  }
  if (text.includes("project") || text.includes("built")) {
    return "Looking up projects...";
  }
  if (text.includes("skill") || text.includes("technology") || text.includes("tech stack")) {
    return "Looking up technical skills...";
  }
  if (text.includes("experience") || text.includes("intern") || text.includes("work")) {
    return "Looking up experience...";
  }
...
export const AIChat = () => {
  ...
  // Scroll conversation container only
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    setError(null);
    setInputValue(""); // Immediate clear input
    ...
    // Set fixed single status text (NO cycling)
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Sorry, I couldn't process that request right now. Please try again.");
      }
              ...
              try {
                ...
                if (parsed.content) {
                  if (!hasReceivedFirstChunk) {
                    ...
                    // REMOVE thinking status completely when first response token arrives
                    setStatusText("");
  ...
  return (
    <>
      {/* ── Floating Toggle Button (STRICT BOTTOM-RIGHT POSITION) ── */}
        ...
        <AnimatePresence>
          {!isOpen && (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="group relative flex items-center gap-2.5 px-4 py-3 rounded-full shadow-2xl cursor-pointer select-none transition-all duration-300"
              style={{
                background: "linear-gradient(135deg, #8254EE 0%, #00C2FF 100%)",
                boxShadow: "0 0 25px rgba(130, 84, 238, 0.45)",
              }}
              aria-label="Open Ask Qayoom AI Chatbot"
            >
              ...
              <span className="text-sm font-bold text-white tracking-wide pr-1">
                Ask Qayoom AI
              </span>
            </motion.button>
          )}
      ...
      {/* ── Chat Modal Drawer (STRICT BOTTOM-RIGHT POSITION ABOVE LAUNCHER) ── */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 right-3 left-3 sm:left-auto sm:right-6 sm:bottom-24 w-auto sm:w-[420px] h-[600px] max-h-[80vh] z-50 flex flex-col rounded-2xl overflow-hidden shadow-2xl chatbot-drawer"
            style={{
              background: "rgba(10, 9, 14, 0.96)",
              border: "1px solid rgba(130, 84, 238, 0.25)",
              backdropFilter: "blur(24px)",
              boxShadow: "0 12px 48px rgba(0, 0, 0, 0.8), 0 0 35px rgba(130, 84, 238, 0.2)",
            }}
          >
            {/* ── Header (AI Avatar & Title on LEFT, Controls on RIGHT) ── */}
            <div
              className="px-4 py-3.5 flex items-center justify-between border-b relative select-none shrink-0"
              style={{
                background: "linear-gradient(135deg, rgba(130, 84, 238, 0.2), rgba(0, 194, 255, 0.1))",
                borderColor: "rgba(130, 84, 238, 0.18)",
              }}
            >
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl" style={{ background: "linear-gradient(135deg, #8254EE, #00C2FF)" }}>
                  <Bot className="w-5 h-5 text-white" />
                </div>
                <div className="text-left">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-sm text-white tracking-wide">Ask Qayoom AI</h3>
                    <span className="px-1.5 py-0.5 text-[10px] font-mono font-medium rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">
                      Nemotron 3
                    </span>
                  </div>
                  <div className="text-[11px]" style={{ color: "#82717B" }}>
                    Ask about Qayoom&apos;s work, projects &amp; skills
                  </div>
                </div>
              </div>
            ...
            {/* ── Conversation Stream Container (ChatGPT/Claude Style) ── */}
            <div
            ...
            >
              {messages.map((msg) => (
                <div key={msg.id} className="w-full">
                  {msg.role === "user" ? (
                    /* User Message: Right Aligned Compact Accent Bubble */
                    <div className="flex justify-end items-start gap-2.5">
                      <div
                        className="max-w-[78%] p-3.5 rounded-2xl rounded-tr-none text-left text-xs sm:text-sm text-white shadow-md"
                        style={{
                    ...
                    /* AI Message: Left Aligned ChatGPT/Claude Clean Response Stream (No Heavy Card Box!) */
              ...
              {/* Single Fixed Thinking Status Indicator (REMOVED completely when response token arrives) */}
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  style={{
                    background: inputValue.trim() && !isLoading ? "linear-gradient(135deg, #8254EE, #00C2FF)" : "rgba(59, 53, 60, 0.4)",
                    color: "#FFFFFF",
                  }}
                >
              ...
              <div className="text-[10px] text-center mt-1.5 opacity-50 font-mono" style={{ color: "#82717B" }}>
